/*
 DeleteAssignmentGUI.java - This class builds the GUI interface for Deleting Assignment. 
 * Date: 14/04/2020
 * @author Fetya Juhar
 *
 ...
 */
public class DeleteAssignmentGUI  extends JInternalFrame  implements ActionListener{
     ...
     */
    DeleteAssignmentGUI(StudentTaskManagmentSystemGUI initSTMSGUI, StudentTaskManagmentSystem initSTMS){
        super("Delete Assignment", true, // resizable
                                    true, // closable
                                    true, // maximizable
                                    true);// iconifiable
    ...
    /* 
     * This method initialise and build the GUI interface, and hook event handler for the Delete assignment button
     * 
    ...
    public JPanel initGUI(){
        ...
        deleteAssignmentButton.addActionListener(this);   // hook the action listener for edit assignment button
        
        JPanel dueDatePanel = new JPanel();
        dueDatePanel.setLayout(new GridLayout(1,3));
        dueDatePanel.add(deleteAssignmentButton);
    ...
    /* 
     * Event handler method of the class for the Delete Assignment button
     * 
    ...
    public void actionPerformed(ActionEvent e) { 
        ...
        if(action.equalsIgnoreCase("Delete Assignment")){               
       
           int input = JOptionPane.showConfirmDialog(null, "Do you want to delete " + String.valueOf(assignmentNameComboBox.getSelectedItem()) + " assignment?");
           
           if(input != 0)
               return ;
           
           deleteStatus = initSTMS.deleteAssignment(assignmentID);   
            
            if(deleteStatus){
                
                JOptionPane.showMessageDialog(this, "Assignment is deleted successfully", "Success Deleting", JOptionPane.INFORMATION_MESSAGE);
                initSTMSGUI.displayLog("NOTIFICATION: " + String.valueOf(assignmentNameComboBox.getSelectedItem()) + " assignment is deleted successfully.");
                initSTMSGUI.disposeInternalFrame("Delete Assignment");  // dispose the Edit Assignment Internal Frame
                populateAssignmentNames();  // re-populate the assignment fields
            }
            else {
                JOptionPane.showMessageDialog(this, "Error occured whilst deleting the assignment.", "Error Deleting", JOptionPane.ERROR_MESSAGE);
                initSTMSGUI.displayLog("ERROR MESSAGE: Error occured whilst deleting the assignment " + String.valueOf(assignmentNameComboBox.getSelectedItem()));
